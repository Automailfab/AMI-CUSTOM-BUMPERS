  ...
  onSummaryClick(event) {
    ...
    if (detailsElement === this.mainDetailsToggle) {
      ...
      setTimeout(() => {
        ...
        !reducedMotion && summaryElement.nextElementSibling.addEventListener('transitionend', addTrapFocus);
      }, 100);
    }
  }
...
class SliderComponent extends HTMLElement {
  ...
  update() {
    // Temporarily prevents unnatural jumps in slider scrolling
    if (!this.slider || !this.nextButton) return;
...
class VariantSelects extends HTMLElement {
  ...
  updateShareUrl() {
    ...
    shareButton.updateUrl(`${window.shopifyVariants[this.dataset.section].url}?variant=${this.currentVariant.id}`);
  }
  ...
  renderProductInfo() {
    ...
    fetch(`${this.dataset.url}?variant=${requestedVariantId}&section_id=${sectionId}`)
      ...
      .then((responseText) => {
        // prevent unnecessary updates when the variant changes again before the request is completed
  ...
  setUnavailable() {
    ...
    const addButton = button.querySelector('button');
